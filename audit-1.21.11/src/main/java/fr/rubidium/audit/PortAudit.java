...
public final class PortAudit implements ModInitializer {
    ...
    public void onInitialize() {
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            ...
            try { Files.writeString(Path.of("port-audit.json"),"{\"minecraft\":\"1.21.11\",\"exactMathCases\":"+cases+",\"passed\":true,\"renderingModified\":false}"); }
            catch(Exception e) { throw new RuntimeException(e); }
            System.out.println("RUBIDIUM_PORT_AUDIT_OK 1.21.11 cases="+cases);
            server.halt(false);
        });
    }
