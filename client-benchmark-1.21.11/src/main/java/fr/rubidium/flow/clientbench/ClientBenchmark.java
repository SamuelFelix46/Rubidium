...
import fr.rubidium.flow.clientbench.mixin.QueueAccessor;
...
public final class ClientBenchmark implements ClientModInitializer {
    ...
    private void tick(Minecraft client) {
        ...
        if(loadedAt==0) {
            ...
            server.execute(() -> {
                ...
                player.teleportTo(server.overworld(),60.5,110,-40.5,Set.of(),-90,25,false);
            });
            return;
        }
        ...
        if(stop!=lastStop) {
            ...
            server.execute(() -> {
                ...
                player.setGameMode(GameType.SPECTATOR);
                player.teleportTo(server.overworld(),32_000+index*256,120,32_000,Set.of(),-90,25,false);
            });
        }
        if(now-lastSample>=1_000_000_000L) {
            lastSample=now;
            var queues=(QueueAccessor)client.levelRenderer.getSectionRenderDispatcher();
            ...
            point.put("pendingUploads",queues.rubidium$benchUploads().size()); point.put("pendingCloses",queues.rubidium$benchCloses().size());
            samples.add(point);
        }
    }
    ...
    private void finish(Minecraft client) {
        ...
        try {
            ...
            report.put("schema",1); report.put("minecraft","1.21.11"); report.put("mode",mode);
            report.put("scenarioRevision",2); report.put("javaRuntime",System.getProperty("java.runtime.version"));
            report.put("gateEnabled",false);
            report.put("scenario","20s spawn warmup, four new terrain stops 15s apart, 60s frame capture");
            report.put("graphics",client.options.graphicsPreset().get().toString());
            report.put("gpu",RenderSystem.getDevice().getRenderer()); report.put("cpu",GLX._getCpuInfo());
            ...
            Screenshot.takeScreenshot(client.getMainRenderTarget(),shot -> {
                try(shot) { shot.writeToFile(client.gameDirectory.toPath().resolve("client-final.png")); }
                catch(Exception error) { throw new RuntimeException(error); }
                finally { client.execute(client::stop); }
            });
